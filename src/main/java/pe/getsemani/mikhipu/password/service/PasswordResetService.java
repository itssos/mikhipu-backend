...
public class PasswordResetService {
    ...
    /**
     * Genera un token único para restablecer contraseña y envía un correo al usuario.
     */
    @Transactional
    public void createPasswordResetToken(String email) {
        Optional<User> optionalUser = userRepository.findByEmail(email.trim());
        if (optionalUser.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No se encontró un usuario con el correo proporcionado.");
        }
    ...
    /**
     * Valida un token activo y actualiza la contraseña del usuario.
     */
    @Transactional
    public void resetPassword(String token, String newPassword) {
        PasswordResetToken resetToken = tokenRepository.findByToken(token.trim())
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "El token de restablecimiento es inválido."));

        if (resetToken.getExpiryDate().isBefore(LocalDateTime.now())) {
            tokenRepository.delete(resetToken); // limpieza de tokens expirados
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "El token de restablecimiento ha expirado.");
        }
    ...
    /**
     * Construye el cuerpo HTML del correo para restablecer contraseña.
     */
    private String buildResetPasswordEmailBody(String resetLink) {
        return String.format("""
            ...
            <body style="font-family:Arial,sans-serif;background:#f4f4f4;padding:0;margin:0">
              <table align="center" width="100%%" style="max-width:600px;background:#fff;border-radius:8px;overflow:hidden">
                ...
                <tr><td style="padding:20px;color:#333">
                  ...
                  <p>Este enlace caduca en %d hora(s). Si no lo solicitaste, ignora este mensaje.</p>
                </td></tr>
            ...
            """, resetLink, EXPIRATION_HOURS, resetLink, resetLink);
    }
}
